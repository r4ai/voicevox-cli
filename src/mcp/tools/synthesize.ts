...
export function registerSynthesizeTool(server: McpServer, defaultHost: string): void {
  server.registerTool(
    "synthesize",
    {
    ...
    async (args) => {
      try {
        ...
        const wav = await client.synthesize(query, args.speaker)

        const outputPath = args.output ?? join(tmpdir(), `voicevox-${Date.now()}.wav`)
